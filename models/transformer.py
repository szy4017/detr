...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_encoder_layers=6,
                 ...
                 return_intermediate_dec=False, sta_query=False):
        super().__init__()
    ...
    def forward(self, src, mask, query_embed, pos_embed):
        ...
        if isinstance(query_embed, dict):
            ...
            # for results_pretrain_state_finetune_5
            # tgt_query_embed = query_embed.weight.unsqueeze(1).repeat(1, bs, 1).to(src.device)
            # sta_query_embed = query_embed.weight.unsqueeze(1).repeat(1, bs, 1).to(src.device)
            # tgt = torch.zeros_like(tgt_query_embed)  # target query
            # sta = torch.zeros_like(sta_query_embed)
        ...
        if self.sta_query:
            ...
            # hs_sta, inter_references = self.sta_decoder(sta.permute(1, 0, 2), reference_points, memory.permute(1, 0, 2), spatial_shapes, level_start_index,
